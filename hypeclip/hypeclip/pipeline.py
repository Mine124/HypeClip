...
def _finish_clip(ctx, start, dur, idx, title, score, settings, r):
    ...
    # ---- Eagle-Eye: inject this clip's camera-follow path ----
...
def _scan_and_render(media_path, dur, title, settings: Settings,
                     r: Reporter, stop):
    """Wizard loop: select -> scan -> intelligence -> [review] -> render."""
    ...
    while True:
        ...
        if sel.get("mode") == "audio":
            ...
            analyzer = scan.ScrollScanner(
                settings, media_path, (rc.get("x", 0.6), rc.get("y", 0),
        ...
        # ---- Eagle-Eye: build camera-follow paths for each clip ----
        if sel.get("mode") == "track" and sel.get("point"):
            try:
                from . import tracker
                pt = sel["point"]
                ordered_m = sorted(moments,
                                   key=lambda x: x.start)[:int(settings.max_clips)]
                tracks = {}
                for i, m in enumerate(ordered_m):
                    ...
                    trk = tracker.build_track(
                        ...
                        (float(pt.get("x", 0.5)), float(pt.get("y", 0.5))),
